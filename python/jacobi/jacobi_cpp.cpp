...
/*
	...
	pybind11::class_<DCMesh >(m, "CMesh")
		.def(pybind11::init<>())
		//.def("read_from_file", &DCMesh::read_from_file)
		.def("print", &DCMesh::print)
		//.def("multiply", &DCMesh::operator*)
		.def("assign",
			static_cast<DCMesh & (DCMesh::*)(const DCMesh&)>
			(&DCMesh::operator=)
		)
		.def("get_data", [](const DCMesh& d)
			{
		...
		.def("set_data", [](DCMesh& d, const pybind11::buffer numpy_matrix)
			{
				...
				d = DCMesh(info.shape[0]); //use assignment to not write other code...
